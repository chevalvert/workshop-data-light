...
if __name__ == '__main__':
  ...
  pixels = image.load()
  author = image.text.get('Author', 'Unknown')
  title = image.text.get('Title', 'Untitled')
  comment = image.text.get('Comment', '')
  description = image.text.get('Description', '')

  frameRateSearch = re.search(r'@([\d\.]+)fps', comment, re.IGNORECASE)
  if frameRateSearch:
  ...
  print('\n\033[4m' + title.strip() + '\033[0m,', author)
  ...
  for x in range(0, image.width):
    ...
    for y in range(0, image.height):
      ...
      b = pixels[x, y][2]
      if (r == g == b):
        strip.setPixelColor(image.height - y, Color(0, 0, 0, r))
      else:
        strip.setPixelColor(image.height - y, Color(g, r, b, 0))
    strip.show()
